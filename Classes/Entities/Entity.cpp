...
void Entity::update(float pDeltaTime)
{
		...
		if(this->mAnimationRunning && (this->mAnimationRepeatCount > 0 || this->mAnimationRepeatCount < 0))
		{
			...
			if(this->mAnimationTimeElapsed >= this->mAnimationTime)
			{
				...
				else
				{
					...
					{
						if(this->mAnimationRepeatCount == -1)
						{
							this->setCurrentFrameIndex(this->mAnimationStartFrame);

							this->mAnimationFramesElapsed = 0;
						}
						else if(this->mAnimationRepeatCount > 0)
						{
							this->mAnimationRepeatCount--;
	...
	if(this->mShadow)
	{
		if(this->mShadow->getParent())
		{
			...
			{
				this->mShadow->setCenterPosition(this->getCenterX(), this->getCenterY() - Utils::coord(40));
			}
		}
	}
	...
	if(this->mIsCollidable)
	{
		...
		{
			float radius = MAX(this->getWidth(), this->getHeight()) + Utils::coord(135.0f);

			float dx = (Options::CAMERA_CENTER_X - this->getCenterX()) * 2.5f;
			float dy = (Options::CAMERA_CENTER_Y - this->getCenterY()) * 2.5f;
			{
				float dist = sqrt(dx * dx + dy * dy);

				dx = Options::CAMERA_CENTER_X - (float) (dx / dist) * radius * 0.4f;
				dy = Options::CAMERA_CENTER_Y - (float) (dy / dist) * radius * 0.4f;
			}

			this->setCenterPosition(dx, dy);
		}

		if((!this->isOnPizza() && this->getZ() <= Options::MIN_Z) || (this->getZ() <= Options::MIN_Z && this->getZOrder() == -2))
		{
...
bool Entity::isOnPizza()
{
	float cx = Options::CAMERA_CENTER_X + 30;
	float cz = Options::CAMERA_CENTER_Y + 30;
	float pizzaRadiusX = 930;
	float pizzaRadiusZ = 500;

	float dx = (this->getCenterX() - cx);
	float dz = (this->getCenterY() - cz) * pizzaRadiusX / pizzaRadiusZ;
